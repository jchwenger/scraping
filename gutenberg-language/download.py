"""
Imported from: https://github.com/getalp/Flaubert
...
def id2url(book_id, encoding="utf-8"):
    ...
    for extension in extensions:
        url = "{root}/{path}/{book_id}{extension}".format(
            root=root, path=subdir_path, book_id=book_id, extension=extension
        )
        # print('url=', url)
        if check_url(url):
            return url
    ...
    if not os.path.exists(os.path.join(indir, "gutenberg_book_urls.txt")):
        ...
        while True:
            ...
            if page_w_books:
                ...
                urls = [
                    el.get("href")
                    for el in page_w_books.select(
                        'body > p > a[href^="http://aleph.gutenberg.org/"]'
                    )
                ]
                ...
                if len(urls) > 0:
                    ...
                    if url_to_next_page[0]:
                        page_book_urls = "http://www.gutenberg.org/robot/" + url_to_next_page[
                            0
                        ].get(
                            "href"
                        )
...
def main():
    ...
    parser.add_argument(
        "-lang", default="fr", type=str, required=False, help="Language to download"
    )
    parser.add_argument(
        "-update_url",
        default=1,
    ...
    page_book_urls = "http://www.gutenberg.org/robot/harvest?filetypes[]=txt&langs[]={lang}".format(
        lang=args.lang
    )
